...
static void engage_z_probe() {
  // Engage Z Servo endstop if enabled
	...
	#else
	  do_blocking_move_to(5, PROBE_DOWN_ENTER, current_position[Z_AXIS] + Z_RAISE_BEFORE_PROBING);
		do_blocking_move_to(0, PROBE_DOWN_ENTER, current_position[Z_AXIS]); // move into ramp
		do_blocking_move_to(0, PROBE_DOWN_EXIT, current_position[Z_AXIS]); // move along ramp to lower probe
		do_blocking_move_to(5, PROBE_DOWN_EXIT, current_position[Z_AXIS]); // move out of ramp
	#endif
}
...
static void retract_z_probe() {
  ...
  if (servo_endstops[Z_AXIS] > -1) {
	...
	#else
	do_blocking_move_to(5, PROBE_UP_ENTER, current_position[Z_AXIS] + Z_RAISE_BEFORE_PROBING);
	do_blocking_move_to(0, PROBE_UP_ENTER, current_position[Z_AXIS]);
	do_blocking_move_to(0, PROBE_UP_EXIT, current_position[Z_AXIS]);
	do_blocking_move_relative(5,0,0);
  #endif
}
  ...
  if(code_seen('G'))
  {
    ...
    case 28: //G28 Home all Axis one at a time
      ...
      if(code_seen(axis_codes[Y_AXIS])) {
        if(code_value_long() != 0) {
          ...
          else if(code_seen(axis_codes[Z_AXIS])) {
            if ( (axis_known_position[X_AXIS]) && (axis_known_position[Y_AXIS]) \
    ...
    case 29: // G29 Detailed Z-Probe, probes the bed at 3 or more points.
      ...
      dock_sled(false);
			...
			for (int yProbe=BACK_PROBE_BED_POSITION; yProbe >= FRONT_PROBE_BED_POSITION; yProbe -= yGridSpacing)
			{
			  int xProbe, xInc;
				if (zig)
				{
...
case 404:  //M404 Enter the nominal filament width (3mm, 1.75mm ) N<3.0> or display nominal filament width 
    ...
    case 851: // M851 Set leveling probe Z offset
    {
      if (code_seen('Z'))
      {
        if ((Z_PROBE_OFFSET_MIN <= code_value()) && (code_value() <= Z_PROBE_OFFSET_MAX))
        {
        ...
        else
        {
          SERIAL_ECHO_START;
					...
					SERIAL_ECHO(Z_PROBE_OFFSET_MIN);
					SERIAL_ECHOPGM(" ");
					SERIAL_ECHOPGM(MSG_Z_MAX);
					SERIAL_ECHOLN(Z_PROBE_OFFSET_MAX);
					break;
        }
      }
      SERIAL_ECHO_START;
